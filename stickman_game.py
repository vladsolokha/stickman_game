...
class MovingPlatformSprite(PlatformSprite):
    def __init__(self, game, photo_image, x, y, width, height):
        #get all characteristics of PlatformSprite
        PlatformSprite.__init__(self, game, photo_image, x, y, width, height)
        #initialize movement
        self.x = 2
        self.counter = 0 #counter used for counting distance moved
        self.last_time = time.time() #start time as last time
        self.width = width 
        self.height = height
        
    def coords(self):
        xy = self.game.canvas.coords(self.image) #get coords position of platform
        self.coordinates.x1 = xy[0] #store left side 
        self.coordinates.y1 = xy[1] #store top side
        self.coordinates.x2 = xy[0] + self.width #store right side
        self.coordinates.y2 = xy[1] + self.height #store bottom
        return self.coordinates
        
    def move(self):
        if time.time() - self.last_time > 0.03: #decide to draw image in 3/100 s in block of code
            self.last_time = time.time() #reset timer to 0
            self.game.canvas.move(self.image, self.x, 0)
            self.counter += 1 #increase step by 1 thus keep moving platform until...
            if self.counter > 20: #checking if step coutner becomes greater than 20 movements
                self.x *= -1 #reverse direction of movement
                self.counter = 0
...
class StickFigureSprite(Sprite):
    def __init__(self, game):
        Sprite.__init__(self, game)
        self.stop_image = PhotoImage(file="stickstop.gif")
        self.images_left = [
        ...
        if self.x != 0 and self.y == 0:
            if time.time() - self.last_time > 0.1:
                ...
                if self.current_image <= 0:
                    self.current_image_add = 1
        if self.x == 0 and self.y == 0:
            self.game.canvas.itemconfig(self.image, image=self.stop_image)        
...
g = Game()
platform1 = MovingPlatformSprite(g, PhotoImage(file="platform1.gif"), 0, 480, 100, 10)
...
platform8 = MovingPlatformSprite(g, PhotoImage(file="platform2.gif"), 45, 60, 66, 10)
